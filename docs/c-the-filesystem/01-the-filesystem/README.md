...
The file was deleted since the /tmp is a temporary directory.
...
It made two files named "touch" and try", you can create a file with a space in it by assigning the name to a variable and then using the command "touch "$[variable]".

### ✅ The root

*Try to create a directory `/backups` (under the root of the filesystem). Why is it failing?*
Because I was denied permission.
...
I can't because I was denied permission.
...
Ln allows you to make links to files, when i use the cat tool I see the contents of "/etc/passwd".
